...
//
// [dir] defaults to the current directory. The Vite root is pinned to this
// package so the client app is served from the install location, while the
// content bundle is whatever directory you point at (exported as OKF_BUNDLE).
...
const COMMANDS = new Set(['dev', 'build', 'graph']);
...
function usage() {
  process.stderr.write(
    `zettel-graph — 3D OKF knowledge-graph visualizer\n\n` +
      ...
      `  zettel-graph graph [dir] -o file   emit graph.json (stdout if no -o)\n\n` +
      `Arguments:\n` +
      `  dir    OKF bundle directory (default: current directory)\n` +
