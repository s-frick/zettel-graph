...
3. **Wire it project-wide.** `capture` only fires across the repo if a root
   ...
   `> This project keeps an OKF knowledge graph in \`knowledge/\`. Follow @knowledge/AGENTS.md and apply its policy proactively as you work.`
...
policy:
  ...
  track: [components, domain-concepts, architecture-decisions, gotchas]
  ...
  capture: auto      # off = only when explicitly asked
  ...
  maintenance:
    ...
    stale:  auto      # detect outdated notes (off | suggest | auto-FLAG only)
    revise: auto      # rewrite notes on new insight (off | suggest | auto)
```
